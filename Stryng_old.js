...
(function (root, factory){
    ...
    var StryngGenerics = {
        ...
        exchange: function(input, oldString, newString, n)
        {
            ...
            else if(n > 0) return Stryng.splitLeft(input, oldString, n);
            else /* if(n < 0) */ return Stryng.splitRight(input, oldString, n);
        },
         ...
         */
        stripLeft: function(input, prefix, limit)
        {
         ...
         * @todo test and benchmark across browsers
         */
        stripLeft2: function(input, prefix, limit)
        {
            limit = toNat(limit, 1/0);
         ...
         * @throws {(Error|TypeError)} if any required argument is missing
         */
        stripRight: function(input, suffix, limit)
        {
            limit = toNat(limit, 1/0);
        ...
        parseQueryString: function(input, arraySuffix, parser)
        {
            ...
            var input = Stryng.stripLeft(input, '?'),
                pairs = input.split('&'),
                params = {};

            while (pairs.length)
            {
                var pair = Stryng.splitLeft(pairs.pop(), '=', 1),

                    key = decodeURIComponent(pair[0]),
                    value = pair[1]; // decoding is the parser's job

                value = parser.call(params, value, key, input);
                key = Stryng.stripRight(key, arraySuffix);
    ...
    Stryng.formatQueryParams = function(object, arraySuffix, formatter)
    {
        ...
        object_forOwn.call(object, function(value, key){
